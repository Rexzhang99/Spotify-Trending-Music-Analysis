...
error_analysis <- function(pred,actual, plot_name) {
  ...
  ggsave(
    file = paste("error", gsub(" ", "", plot_name), '.pdf', sep = ''),
    path = "Thesis/figure",
...
table(DT.train$popularity)
# data is not balanced, we first balance our train data 
DT.test=caret::downSample(x=DT.test[, -ncol(DT.test)],y=DT.test$popularity,yname="popularity")
DT.train=caret::downSample(x=DT.train[, -ncol(DT.train)],y=DT.train$popularity,yname="popularity")
...
error.forest=error_analysis(as.vector(pred.tree),DT.test[, 14],plot_name="Random Forest")
error.forest
